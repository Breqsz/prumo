...
// Shader de ferrofluido, portado do React Bits (MIT) para TypeScript.
// Substitui os vídeos de fundo ambient: mesma linguagem visual do
// `ambient-2.mp4` que existia antes, por alguns KB em vez de 8 MB, e
// rodando também no celular — onde o vídeo era suprimido de propósito.
// Cores default em branco puro, como manda o ADR 0001: hierarquia por
// opacidade, nunca por hue decorativa.
...
const flowVec = (d: FlowDirection): [number, number] => {
  switch (d) {
    case 'up':
      return [0, 1];
    case 'down':
...
export type FerrofluidProps = {
  ...
  dpr?: number;
  paused?: boolean;
  /** Default branco puro: o ADR 0001 proíbe hue decorativa. */
  colors?: string[];
  speed?: number;
  scale?: number;
  turbulence?: number;
...
export const Ferrofluid = ({
  ...
  useEffect(() => {
    ...
    // Sem WebGL o fundo simplesmente não existe — e é só isso que pode
    // acontecer. Contexto negado (GPU em lista de bloqueio, browser antigo,
    // aba em segundo plano perdendo o contexto) não pode derrubar a página:
    // as camadas de véu, grão e fades já compõem um fundo completo sozinhas.
    ...
    return () => {
      ...
      const callIfFn = (obj: unknown, key: string) => {
        const fn = obj ? (obj as Record<string, unknown>)[key] : undefined;
